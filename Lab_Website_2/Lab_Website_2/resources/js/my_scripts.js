...
/*
	Registration Page:legend(hp, 'position (m)', 'velocity (m/s)', ...
               'acceleration (m/s^2)', 'location', 'southwest')
    title('Free Fall')
    xlabel('time (s)')
...
*/
	function loadStatsPage(){
		...
		for(row_counter = 2; row_counter < table.rows.length; row_counter++){
			console.log(table.rows[row_counter].cells[2].innerHTML);
			...
			if(parseInt(table.rows[row_counter].cells[2].innerHTML) > parseInt(table.rows[row_counter].cells[3].innerHTML)){
				...
				winning.innerHTML = parseInt(winning.innerHTML) + 1;
			}
			else
			{
				console.log("CU Loses");
				table.rows[row_counter].cells[4].innerHTML =	table.rows[row_counter].cells[1].innerHTML;
				lossing.innerHTML = parseInt(lossing.innerHTML) + 1;
			}
		}
	}
/*
...
*/
	...
	function loadPlayersPage(){
		var selecter = document.getElementById("player_selector");
		for(var i = 0; i < players.length; i++){
			var play    	=	players[i].name;
			var aTag = document.createElement('a')
			aTag.setAttribute('href',"#");
			aTag.innerText = play
			var funstring = "switchPlayers(" + i + ")";
			aTag.setAttribute('onclick', funstring);
			...
			sel.textContent	=	play;
			sel.value       =	play;
			sel.onclick = switchPlayers(i);
			// sel.addEventListener("onclick", players[i].a);
			selecter.appendChild(sel);
			// selecter.innerHTML = players[i].a;
			// selecter.onclick = players[i].a;
			// selecter.option.add(sel);
